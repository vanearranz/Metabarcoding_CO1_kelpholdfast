########################################################
###### Script to perform the analysis of the manuscript "Metabarcoding hyperdiverse marine communities in temperate kelp forests: an experimental approach to inform future studies." TEST-Dave
########################################################
...
load("Resources/Edesign_github.Rdata")
...
## Before running the function to obtain the results of our test, we need to prepare the sample data, contrast matrix and the samples with its corresponding levels 

# 1. Add the Sample_data to merge the corresponding samples
sample_data_tomerge <- read.csv(file = "Resources/sample_data_tomerge_edit.csv")
sample_data_tomerge

# 2. Import all the samples with its corresponding level SPEF as made them factors
Design_long_format <- read.csv("Resources/Design_long_format.csv")
#Design_long_format <- droplevels(Design_long_format)
Design_long_format$Smp <- as.factor(Design_long_format$Smp)
Design_long_format$SFEP <- as.factor(Design_long_format$SFEP)
Design_long_format

# 3. Contrast matrix for the comparisons 

# Import the contrast matrix with the hypothesis and the number of corresponding to each factor SFEP
Design_w_levels <- read.delim("Resources/Design_w_levels.csv", stringsAsFactors = TRUE)
# Define the contrast matrix with our design to create a square contrast matrix
contrasts(Design_long_format$SFEP) <- as.matrix(Design_w_levels[,-1])
...
ASV_nf.LM.allr.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_nf.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_nf.LM.ass.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_nf_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

OTU_nofilter
OTU_nofilter.out  <- Edesign.merging(phyloseq.obj.raw = OTU_nofilter, sample.data = sample_data_tomerge)
OTU_nf.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_nofilter.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
OTU_nf.LM.ass.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_nf_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

#### Results LULU ###### 

ASV_lulu
ASV_lulu.out  <- Edesign.merging(phyloseq.obj.raw = ASV_lulu, sample.data = sample_data_tomerge)
ASV_lulu.LM.allr.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_lulu.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_lulu.LM.ass.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_lulu_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

OTU_lulu
OTU_lulu.out <- Edesign.merging(phyloseq.obj.raw = OTU_lulu, sample.data = sample_data_tomerge)
OTU_lulu.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_lulu.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
OTU_lulu.LM.ass.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_lulu.ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

#### Results Filtering 0.003% ###### 

ASV_f1
ASV_f1.out  <- Edesign.merging(phyloseq.obj.raw = ASV_f1, sample.data = sample_data_tomerge)
ASV_f1.LM.allr.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_f1.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_f1.LM.ass.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_f1_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

OTU_f1
OTU_f1.out <- Edesign.merging(phyloseq.obj.raw = OTU_f1, sample.data = sample_data_tomerge)
OTU_f1.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_f1.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
OTU_f1.LM.ass.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_f1.ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

#### Results Filtering 0.01% ###### 

ASV_f2
ASV_f2.out  <- Edesign.merging(phyloseq.obj.raw = ASV_f2, sample.data = sample_data_tomerge)
ASV_f2.LM.allr.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_f2.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_f2.LM.ass.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_f2_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 2000)

OTU_f2
OTU_f2.out <- Edesign.merging(phyloseq.obj.raw = OTU_f2, sample.data = sample_data_tomerge)
OTU_f2.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_f2.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_f3.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = ASV_f3.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
...
ASV_f3.LM.ass.out  <- Edesign.LM.pipeline(phyloseq.obj = ASV_f3_ass.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 1500)

OTU_f3
OTU_f3.out  <- Edesign.merging(phyloseq.obj.raw = OTU_f3, sample.data = sample_data_tomerge)
OTU_f3.LM.allr.out <- Edesign.LM.pipeline(phyloseq.obj = OTU_f3.out, contrast.mat = square_cm4, design = Design_long_format, R.size = 4000)
