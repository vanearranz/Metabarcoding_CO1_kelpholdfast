### Script to Plot Figures 3 and 4 in manuscript "Metabarcoding hyperdiverse marine communities in temperate kelp forests: an experimental approach to inform future studies." by Vanessa Arranz, Libby Liggins and J. David Aguirre. 
library(phyloseq)
library(lme4)
library(lmerTest)
library(vegan)
...
newcontmatrix <- cbind(1, square_cm3)
...
LF.df$contrast <- factor(LF.df$contrast, levels=unique(LF.df$contrast))

#To set up the panels
LF.df$panel <- "Sample Preparation"
LF.df$panel[11:16] <- "DNA Extraction"
LF.df$panel[17:22] <- "PCR Amplification"
