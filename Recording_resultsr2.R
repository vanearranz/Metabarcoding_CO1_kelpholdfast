...
for(j in 1:length(results.list)){
  ...
  tile.sum.tmp <- list()
    ...
    tile.df <- data.frame(R.seed = names(ASV_f1.LM.allr.out)[i],  Model = factor(rep(c("Richness", "Composition", "Dispersions"), each = 11), levels = (c("Richness", "Composition", "Dispersions"))), contrast = factor(rep(names(permdisp.col),3), levels = (names(permdisp.col))), Sig. = c(lm.col, adonis.col, permdisp.col))  
...
SM_A2 <- ggplot(tile.sum.all.p)+
  ...
  theme(strip.text.y = element_text(angle = 0, size = 6), axis.text.y = element_blank(), axis.title.y = element_blank(), axis.text.y.left = element_blank(), axis.title.y.left = element_blank(), axis.title.x=element_blank(), axis.text.x=element_blank(), panel.spacing.y = unit(0.05, "lines")) 
